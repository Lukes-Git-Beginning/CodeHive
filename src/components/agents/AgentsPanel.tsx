...
export function AgentsPanel() {
  ...
  return (
    <div className="p-4 space-y-4">
      {/* Active Agents */}
      <div>
        <h3 className="font-label text-sm text-text-primary mb-3">
          {agents.length > 0 ? `Aktive Agenten (${agents.length})` : 'Keine aktiven Agenten'}
        </h3>
      ...
      {/* Available Roles */}
      <div>
        <h3 className="font-label text-sm text-text-primary mb-3">Verfügbare Rollen</h3>
        <div className="grid grid-cols-4 gap-2">
          {Object.entries(ROLE_CONFIG).map(([role, config]) => {
            const Icon = config.icon
            return (
              <div key={role} className="card-surface rounded-lg p-3 flex flex-col items-center gap-1.5 text-center">
